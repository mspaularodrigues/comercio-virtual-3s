...
<div id="carouselCards" class="carousel slide" data-bs-ride="carousel" data-bs-wrap="true">
  ...
  <div class="carousel-inner">
    @foreach($produtosSimilares as $index => $product)
      <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
        <div class="d-flex justify-content-center">
          {{-- Mostrar 3 cards a partir do index atual --}}
          @for ($i = 0; $i < 3; $i++)
            @php
              $pos = ($index + $i) % $produtosSimilares->count();
              $p = $produtosSimilares[$pos];
            @endphp
            <div class="card mx-2" style="flex: 0 0 200px; max-width: 200px; text-align: center;">
              <img src="{{ $p->image_url }}" class="card-img-top" alt="{{ $p->name }}" style="width: auto; max-width: 60%; padding-top: 12px; height: auto; display: block; margin-left: auto; margin-right: auto;">
              <div class="card-body">
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const totalSlides = {{ $produtosSimilares->count() }};
